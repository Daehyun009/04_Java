...
//		else if (age <= 19)	result = "청소년(고)"; 두 줄을 아래 삼항연산자로 처리
 ...
 * 
	...
	public void method5() {
		...
		int num1 = sc.nextInt();
		double sum1 = num1 * 0.4;
		
		System.out.print("기말고사 점수 (50%) : ");
		int num2 = sc.nextInt();
		double sum2 = num2 * 0.5;
		
		System.out.print("과제 점수 (10%) : ");
		int num3 = sc.nextInt();
		double sum3 = num3 * 0.1;
...
// 방법4
		...
		System.out.printf("%s의 최종 점수 : %.1f점", name, sum);
		System.out.println();
	...
	public void practice() {
		...
		String result = "";
		double avg = (kor + eng + mat + soc + sci) / 5;
		...
		if (kor < 40 || eng < 40 || mat < 40 || soc < 40 || sci < 40) {
			...
			System.out.println("FAIL [40점 미만 과목 :" + result + "]");
			return;
		}
		if (0 <= avg && avg < 60.0) {
			System.out.println("FAIL [점수 : " + avg + " (평균 미달)]");
		}else if (avg <= 100) {
...
// 방법 1
	/*
	if(0 <= sum || sum < 60) result = "F";
	else if (sum < 65) result = "D";
	else if (sum < 70) result = "D+";
	else if (sum < 75) result = "C";
	else if (sum < 80) result = "C+";
	else if (sum < 85) result = "B";
	else if (sum < 90) result = "B+";
	else if (sum < 95) result = "A";
	else if (sum <= 100) result = "A+";
	else result = "입력 오류";
	*/
